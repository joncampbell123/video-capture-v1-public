/* RIFF:AVI writer general implementation.
 * (C) 2008-2015 Jonathan Campbell
 *
 * Supports:
 *   - Multiple AVI streams, audio & video
 *   - Automatic updating of the dwLength and dwTotalFrame fields
 *   - Optional compatibility with traditional AVI applications
 *   - Support for OpenDML 2.0 AVI format and the ability to write
 *     AVI files >= 2GB with index
 *
 * Todo:
 *   - Go through the code when you can and make sure structures
 *     are written using __w_le_xxx() and read using __le_xxx() to
 *     ensure that the output is the same even on big Endian machines
 *
 *   - Slowly convert assertion-based riff_stack_write(), etc. to
 *     code that properly bails out and returns an error if the
 *     write() function fails.
 * 
 * NOTES:
 *   - All structures you pass in are expected to have been
 *     written as little endian WORDs and DWORDs, this impl.
 *     will not byte-swap them for you
 *
 *   - Stream data will also not be byte-swapped for you,
 *     keep that in mind if you are writing PCM audio
 *
 *   - This implementation makes no attempt to optimize or
 *     combine/split any video/audio data written to the stream.
 *     If your implementation writes PCM audio 4 bytes at at time,
 *     you will get an AVI file with 4 byte long audio chunks and
 *     an index for all those chunks---a major waste of disk space!
 *     YOU are responsible for writing reasonable-sized chunks of
 *     combined samples to disk!
 *
 *   - This code generally mimics the way FFMPEG/mplayer writes
 *     OpenDML AVI files, including the use of "JUNK" chunks in
 *     the LIST:strl header chunks in case the OpenDML index
 *     may be needed. But there is one difference: FFMPEG writes
 *     the ix## chunk when terminating the movi chunk at the 1GB
 *     boundary (when splitting off to the next RIFF:AVIX), meaning
 *     that in every movi chunk there is ix## at the end, while
 *     this implementation waits until the caller is done to write
 *     all the ix## chunks in one go at the end of the AVI. Since
 *     the OpenDML structure uses offsets in the file to refer to
 *     ix## chunks, this makes no difference whatsoever to the end
